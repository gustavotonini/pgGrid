...
set_plain_rel_pathlist(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte)
{
	...
	if (IsRelationFragmented(rte->relid) && (PGR_Is_Replicated_Query == false) ){
		elog(NOTICE, "PGGRID: fragmented relation. Consider only cluster scan1");
		/* Consider just cluster scan */
		add_path(rel, create_clusterscan_path(root, rel));
	}else{
					  ...
					  bool *differentTypes)
{
	...
	{
		elog(ERROR, "unrecognized node type(1): %d",
			 (int) nodeTag(setOp));
	}
	return true;
}
				  ...
				  RangeTblEntry *rte, Index rti, Node *qual)
{
	...
	{
		elog(ERROR, "unrecognized node type(2): %d",
			 (int) nodeTag(setOp));
	}
}
