...
/*
 * cost_clusterscan
 *	  Determines and returns the cost of scanning a relation in cluster
 */
void
cost_clusterscan(Path *path, PlannerInfo *root,
			 RelOptInfo *baserel)
{
	path->startup_cost = INFINITY;
	path->total_cost = INFINITY;
}
...
get_indexpath_pages(Path *bitmapqual)
{
	...
	else if (IsA(bitmapqual, IndexPath))
	{
		...
		elog(ERROR, "unrecognized node type(3): %d", nodeTag(bitmapqual));
...
cost_bitmap_tree_node(Path *path, Cost *cost, Selectivity *selec)
{
	...
	{
		elog(ERROR, "unrecognized node type(4): %d", nodeTag(path));
