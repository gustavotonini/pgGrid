...
static ClusterScan *create_clusterscan_plan(PlannerInfo *root, Path *best_path,
					List *tlist, List *scan_clauses);
...
static ClusterScan *make_clusterscan(List *qptlist, List *qpqual, Index scanrelid);
...
create_plan(PlannerInfo *root, Path *best_path)
{
	...
	switch (best_path->pathtype)
	{
		...
		case T_Unique:
			...
			break;
		default:
			elog(ERROR, "unrecognized node type(32): %d",
...
create_scan_plan(PlannerInfo *root, Path *best_path)
{
	...
	{
		case T_SeqScan:
			elog(NOTICE, "PGGRID: criando plano seq");
			...
			break;
		case T_ClusterScan:
			elog(NOTICE, "PGGRID: criando plano cluster");
			plan = (Plan *) create_clusterscan_plan(root,
												best_path,
												tlist,
												scan_clauses);
			break;
		...
		default:
			elog(ERROR, "unrecognized node type(33): %d",
				 (int) best_path->pathtype);
			plan = NULL;		/* keep compiler quiet */
			break;
	}
...
create_join_plan(PlannerInfo *root, JoinPath *best_path)
{
	...
	switch (best_path->path.pathtype)
	{
		...
		case T_NestLoop:
			...
			break;
		default:
			elog(ERROR, "unrecognized node type(34): %d",
...
static ClusterScan *
create_clusterscan_plan(PlannerInfo *root, Path *best_path,
					List *tlist, List *scan_clauses){
	ClusterScan    *scan_plan;
	Index		scan_relid = best_path->parent->relid;
	...
	scan_plan = make_clusterscan(tlist,
							 scan_clauses,
							 scan_relid);
					  ...
					  List **qual, List **indexqual)
{
	...
	{
		elog(ERROR, "unrecognized node type(35): %d", nodeTag(bitmapqual));
		plan = NULL;			/* keep compiler quiet */
	}
...
static ClusterScan *
make_clusterscan(List *qptlist,
                         List *qpqual,
                         Index scanrelid)
{
        ClusterScan    *node = makeNode(ClusterScan);
        Plan       *plan = &node->plan;
