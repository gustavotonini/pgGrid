...
/*
 * create_clusterscan_path
 *	  Creates a path corresponding to a full cluster scan, returning the
 *	  pathnode.
 */
Path *
create_clusterscan_path(PlannerInfo *root, RelOptInfo *rel)
{
	Path	   *pathnode = makeNode(Path);

	pathnode->pathtype = T_ClusterScan;
	pathnode->parent = rel;
	pathnode->pathkeys = NIL;	/* clusterscan has unordered result */

	cost_clusterscan(pathnode, root, rel);
