...
ExecInitNode(Plan *node, EState *estate, int eflags)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_BitmapOr:
			 ...
			 */
		case T_ClusterScan:
			//elog(NOTICE, "PGGRID: executando cluster scan");
			result = (PlanState *) ExecInitClusterScan((ClusterScan *) node, estate, eflags);
			break;
		case T_SeqScan:
		...
		default:
			elog(ERROR, "unrecognized node type(5): %d", (int) nodeTag(node));
...
ExecProcNode(PlanState *node)
{
	...
	if (node->instrument)
		InstrStartNode(node->instrument);
				...
				//elog(NOTICE, "PGGRID: execprocnode switch");
			//elog(NOTICE, "node type(6): %d", (int) nodeTag(node));
	switch (nodeTag(node))
	{
			/*
			 * control nodes
		...
		case T_AppendState:
			 ...
			 */
		case T_SeqScanState:
				//elog(NOTICE, "PGGRID: ExecSeqScan");
			result = ExecSeqScan((SeqScanState *) node);
			break;
		case T_ClusterScanState:
				//elog(NOTICE, "PGGRID: ExecClusterScan");
		...
		default:
			//elog(ERROR, "unrecognized node type(6): %d", (int) nodeTag(node));
			result = NULL;
			break;
	}
...
MultiExecProcNode(PlanState *node)
{
	...
	switch (nodeTag(node))
	{
		...
		default:
			elog(ERROR, "unrecognized node type(7): %d", (int) nodeTag(node));
			result = NULL;
			break;
	}
...
ExecCountSlotsNode(Plan *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_ClusterScan:
			return ExecCountSlotsClusterScan((ClusterScan *) node);
		...
		default:
			elog(ERROR, "unrecognized node type(8): %d", (int) nodeTag(node));
			break;
	}
...
ExecEndNode(PlanState *node)
{
	...
	switch (nodeTag(node))
	{
		...
		default:
			elog(ERROR, "unrecognized node type(9): %d", (int) nodeTag(node));
			break;
	}
}
