...
ExecInitExpr(Expr *node, PlanState *parent)
{
	...
	switch (nodeTag(node))
	{
		...
		default:
			elog(ERROR, "unrecognized node type(qual): %d",
				 (int) nodeTag(node));
...
ExecPrepareExpr(Expr *node, EState *estate)
{
	...
	//elog(NOTICE, "before fix");
	fix_opfuncids((Node *) node);
	//elog(NOTICE, "after fix");

	oldcontext = MemoryContextSwitchTo(estate->es_query_cxt);
	//elog(NOTICE, "after switch");
	result = ExecInitExpr(node, NULL);
	//elog(NOTICE, "after ExecInit");
...
ExecQual(List *qual, ExprContext *econtext, bool resultForNull)
{
	...
	foreach(l, qual)
	{
		...
		//elog(NOTICE, "PGGRID :avaliando");
		char *teste = nodeToString(clause);
		//elog(NOTICE, teste);	

		expr_value = ExecEvalExpr(clause, econtext, &isNull, NULL);
		//elog(NOTICE, "PGGRID :avaliou?");
		if (isNull)
		{
