...
ExecReScan(PlanState *node, ExprContext *exprCtxt)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_ClusterScanState:
                        ExecClusterScan((ClusterScanState *) node, exprCtxt);
                        break;
		...
		default:
			elog(ERROR, "unrecognized node type(aqui1): %d", (int) nodeTag(node));
			break;
	}
...
ExecMarkPos(PlanState *node)
{
	switch (nodeTag(node))
	{
		...
		default:
			...
			elog(DEBUG2, "unrecognized node type(aqui2): %d", (int) nodeTag(node));
			break;
	}
}
...
ExecRestrPos(PlanState *node)
{
	switch (nodeTag(node))
	{
		...
		default:
			elog(ERROR, "unrecognized node type(aqui3): %d", (int) nodeTag(node));
			break;
	}
}
