			...
			ScanDirection direction, long count)
{
	...
	if (ScanDirectionIsNoMovement(direction))
		result = NULL;
			...
			//elog(NOTICE, "PGGRID: ExecutePlan");
		result = ExecutePlan(estate,
							 ...
							 direction,
							 dest);
			//elog(NOTICE, "PGGRID: ExecutePlan fim");
	}

	/*
	 * shutdown tuple receiver, if we started it
	 */
			//elog(NOTICE, "PGGRID: enviando tuplas");
	if (sendTuples)
		(*dest->rShutdown) (dest);
			//elog(NOTICE, "PGGRID: enviando tuplas fim");
	MemoryContextSwitchTo(oldcontext);
	return result;
}
...
lnext:	;
		...
		{
				//elog(NOTICE, "PGGRID: EvalQualNext");
			planSlot = EvalPlanQualNext(estate);
				//elog(NOTICE, "PGGRID: EvalQualNext fim");
			if (TupIsNull(planSlot))
				planSlot = ExecProcNode(planstate);
		}
		else{
				//elog(NOTICE, "PGGRID: ExecProcNode");
			planSlot = ExecProcNode(planstate);
				//elog(NOTICE, "PGGRID: ExecProcNode fim");
		}
					...
					switch (test)
					{
						...
						case HeapTupleUpdated:
												   ...
												   &tuple.t_self))
							{
								...
								//elog(NOTICE, "PGGRID: EvalQual");
								newSlot = EvalPlanQual(estate,
													   erm->rti,
													   &update_ctid,
													   update_xmax);
								//elog(NOTICE, "PGGRID: EvalQual fim");
		...
		{
			case CMD_SELECT:
				//elog(NOTICE, "PGGRID: ExecSelect");
				ExecSelect(slot, dest, estate);
				//elog(NOTICE, "PGGRID: ExecSelect fim");
				result = slot;
				break;
...
ExecSelect(TupleTableSlot *slot,
		   ...
		   EState *estate)
{
				//elog(NOTICE, "PGGRID: movendo tupla");
	(*dest->receiveSlot) (slot, dest);
				//elog(NOTICE, "PGGRID: movendo tupla fim");
	IncrRetrieved();
	(estate->es_processed)++;
}
		   ...
		   EState *estate)
{
	...
	#ifdef PGGRID		
    if (!ReplicateOperation(estate->es_per_tuple_exprcontext, tuple, resultRelationDesc, CMD_INSERT)){
		//coordinates grid rollback
	}
	if (ReplicateLastCommandLocally())
	#endif
	{
