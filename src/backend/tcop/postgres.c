...
exec_simple_query(const char *query_string)
{
	...
	if (PGR_Is_Replicated_Query == false)
	{
		...
		/*if (PGR_Is_Replicated_Query)
			elog(NOTICE, "Its replicated query");
		else
			elog(NOTICE, "Its not replicated query");*/
	}
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		else
		{
			...
			{
				elog(NOTICE, "Replicando");
				status = PGR_replication(query_ptr,dest,parsetree,commandTag,"0",0);
				elog(NOTICE, "Ja Replicado");
				if (status == STATUS_REPLICATED)
				{
					elog(NOTICE, "PGGRID: Rep OK");
					if (xact_started)
					{
						finish_xact_command();
						xact_started = false;
				...
				{
					//elog(NOTICE, "PGGRID: Continuar");
					PGR_Check_Lock.dest = TO_FRONTEND;
				}
				else
				{
					//elog(NOTICE, "PGGRID: TO SERVER");
...
Skip_Replication:
		...
		CHECK_FOR_INTERRUPTS();
		//elog(NOTICE, "PGGRID: Executando consulta");
		/*
		...
		//elog(NOTICE, "PGGRID: Executando de verdade");
		 ...
		 */
		//elog(NOTICE, "PGGRID: PortalStart?");
		...
		//elog(NOTICE, "PGGRID: PortalSetResult?");
		...
		//elog(NOTICE, "PGGRID: receiver?");
		(*receiver->rDestroy) (receiver);

		//elog(NOTICE, "PGGRID: PortalDrop?");
		PortalDrop(portal, false);
		//elog(NOTICE, "PGGRID: resultado?");
		if (IsA(parsetree, TransactionStmt))
		{
			/*
